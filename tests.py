...
if True:    # Тест проверки функции нахождения суммы
    data = [-2.1, 2.1, -4.1, 4.1]
    assert isclose(0, summ(data))

if True:    # Тест проверки функции нахождения произведения
    data = [-2, 21, -4.1, 8]
    assert isclose(1377.6, product(data))

if True:    # Тест проверки файла на скорость при увеличении количества данных в файле в 5000 раз
    try:
...
if True:    # Тест проверки всех функций на рандомном количестве рандомных чисел 1000 раз подряд
    for i in range(1000):
